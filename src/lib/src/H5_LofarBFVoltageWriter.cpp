    ...
    switch (_nBits) {
        case 32: {
             for (unsigned t = 0; t < nSamples; ++t) {
                 for (unsigned p = 0; p < polsToWrite(); p+=2) {
                     for (int s = nSubbands - 1; s >= 0 ; --s) {
                         long index = spec->index(s, nSubbands, 
                                 p, nPolarisations, t, nChannels );
                         for(int i = nChannels - 1; i >= 0 ; --i) {
                             _file[p]->write(reinterpret_cast<const char*>(&data[index + i].real()), sizeof(float));
                             _file[p+1]->write(reinterpret_cast<const char*>(&data[index + i].imag()), sizeof(float));
                         }
                     }
                 }
             }
